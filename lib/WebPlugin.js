...
class WebPlugin {
	...
	apply(compiler) {
		...
		compiler.plugin('emit', (compilation, done) => {
			...
			let htmlOut = this.htmlDocument.serialize();

			if(typeof options.stripTag !== undefined) {
				for(var tag in options.stripTags) {
					let head = new RegExp("/<" + tag + "([^>]*)?>/i");
					let tail = new RegExp("/<\/" + tag + ">/i");

					htmlOut = htmlOut.replace(head, "")
									 .replace(tail, "");
				}
			}
			
			// if(options.htmlTag === false) {
			// 	//strip html & empty head
			// 	htmlOut = htmlOut.replace(/<html([^>]*)?>/i, "").replace(/<\/html>/i, "");
			// }

			// if(options.headTag === false) {
			// 	htmlOut = htmlOut.replace("<head></head>", "");
			// }

			// if(options.bodyTag === false) {
			// 	htmlOut = htmlOut.replace(/<body([^>]*)?>/i, "").replace(/<\/body>/i,"");
			// }

			htmlOut = entities.decode(htmlOut.replace(/^\s*[\r\n]/gm, ""));
